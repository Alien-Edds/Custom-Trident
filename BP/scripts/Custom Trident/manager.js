...
export class TridentManager {
    ...
    static getKnockbackMultiplier(player) {
        let multiplier = 1.0;
    ...
    static async isInEnvironment(environment, player) {
        switch (environment) {
            ...
            case RiptideEnvironment.Water:
                if (player.isInWater)
                    return true;
                if (await this.isInWaterOrRain(player.dimension, player.location))
                    return true;
                break;
            case RiptideEnvironment.OnFire:
                ...
                let block = undefined;
                try {
                    block = player.dimension.getBlock(player.location);
                }
                catch { }
                if (block?.isLiquid && !block.typeId.includes("water"))
                    return true;
                break;
        }
        return false;
    ...
    static async isInWaterOrRain(dimension, location) {
        let inWaterOrRain = false;
        ...
        await new Promise((resolve) => {
            ...
            const event = system.afterEvents.scriptEventReceive.subscribe((data) => {
                if (!data.sourceEntity || data.id !== "custom_trident:in_water_or_rain")
                    return;
                if (data.sourceEntity.id !== entity.id)
                    return;
    ...
    static async riptide(tridentData, enchComp, source) {
        if (!tridentData.riptide)
            return;
        let riptideLevel = undefined;
